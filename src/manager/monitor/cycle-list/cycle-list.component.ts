...
import { MatSelectionListChange, MatChipSelectionChange } from '@angular/material';
...
export class CycleListComponent implements OnInit {
    ...
    data.forEach(item => {
      if (item.groupName.length > 1) {
        ...
        if (item.operation == '0' && index == -1) {
          ...
          if (this.cycleNodes.length > index && this.cycleNodes[index].isActive) {
            this.cycleDataNodes = [];
          }
  ...
  selectionChange(item: MatChipSelectionChange) {
